...
const HistoryPage = ({ onViewFile }) => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen p-6 bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50 flex items-center justify-center">
        <div className="text-center space-y-6">
          ...
          <p className="text-slate-700 text-xl font-semibold">Loading portfolio history...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50">
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Header */}
        <div className="relative overflow-hidden bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 rounded-3xl shadow-2xl p-10">
        ...
        {history.length === 0 ? (
          <div className="bg-white rounded-3xl shadow-xl border-2 border-slate-200 p-16 text-center space-y-6">
            <div className="inline-flex items-center justify-center w-24 h-24 bg-gradient-to-br from-slate-100 to-slate-200 rounded-3xl">
            ...
            <div>
              <h3 className="text-3xl font-bold text-slate-800 mb-2">No History Yet</h3>
              <p className="text-lg text-slate-600">Upload and extract your first financial document to build your history</p>
            </div>
          </div>
        ) : (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 to-cyan-600 rounded-2xl shadow-xl p-8 text-center">
              ...
              <div className="relative overflow-hidden bg-gradient-to-br from-purple-600 to-pink-600 rounded-2xl shadow-xl p-8 text-center">
                <div className="absolute top-4 right-4 opacity-20">
                  <svg className="w-16 h-16 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="text-white/90 font-semibold text-lg">Sheets Generated</div>
              </div>
              
              <div className="relative overflow-hidden bg-gradient-to-br from-emerald-600 to-teal-600 rounded-2xl shadow-xl p-8 text-center">
                <div className="absolute top-4 right-4 opacity-20">
                  <svg className="w-16 h-16 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="bg-white rounded-3xl shadow-xl border-2 border-slate-200 overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="bg-gradient-to-r from-slate-100 via-blue-50 to-slate-100 border-b-2 border-slate-200">
                      <th className="px-6 py-5 text-left text-sm font-bold text-slate-800 uppercase tracking-wide">Document</th>
                      <th className="px-6 py-5 text-left text-sm font-bold text-slate-800 uppercase tracking-wide">Date & Time</th>
                      <th className="px-6 py-5 text-left text-sm font-bold text-slate-800 uppercase tracking-wide">Duration</th>
                      <th className="px-6 py-5 text-left text-sm font-bold text-slate-800 uppercase tracking-wide">Data Points</th>
                      <th className="px-6 py-5 text-left text-sm font-bold text-slate-800 uppercase tracking-wide">Sheets</th>
                      <th className="px-6 py-5 text-left text-sm font-bold text-slate-800 uppercase tracking-wide">AI Model</th>
                      <th className="px-6 py-5 text-left text-sm font-bold text-slate-800 uppercase tracking-wide">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {currentItems.map((item, idx) => (
                      <tr key={item.id} className={`border-b border-slate-100 hover:bg-blue-50/50 transition-colors duration-150 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-slate-800 font-semibold text-sm">{item.original_filename || 'Unknown'}</span>
                          </div>
                        </td>
                        <td className="px-6 py-5 text-slate-600 text-sm font-medium">{formatDate(item.extraction_timestamp)}</td>
                        <td className="px-6 py-5">
                        ...
                        <td className="px-6 py-5 text-slate-600 text-sm font-semibold">{(item.total_characters_extracted || 0).toLocaleString()}</td>
                        <td className="px-6 py-5">
                          <span className="inline-flex items-center gap-1 px-3 py-1 bg-purple-100 text-purple-700 rounded-lg text-sm font-semibold">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7m0 10a2 2 0 002 2h2a2 2 0 002-2V7a2 2 0 00-2-2h-2a2 2 0 00-2 2" />
                            </svg>
                            {item.total_sheets_generated || 0}
                          </span>
                        </td>
                        <td className="px-6 py-5">
